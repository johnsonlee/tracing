...
class LooperUtilTest {
    ...
    @Test
    fun `build stack trace from handler`() {
        val trace = buildStackTrace(">>>>> Dispatching to Handler (android.app.ActivityThread\$H) {33e0fae} null: 1")
        assertEquals(1, trace.size)
        val handler = trace.first()
        assertEquals(ActivityThread.H::class.java.name, handler.className)
        assertEquals("Handler.java", handler.fileName)
        assertEquals("handleMessage", handler.methodName)
        assertEquals(-1, handler.lineNumber)
    }

    @Test
    fun `build stack trace from handler and callback`() {
        val trace = buildStackTrace(">>>>> Dispatching to Handler (android.app.ActivityThread\$H) {33e0fae} android.view.ViewRootImpl$4@33e0fae: 1")
        assertEquals(2, trace.size)

        val callback = trace.first()
        assertEquals("android.view.ViewRootImpl\$4", callback.className)
        assertEquals("ViewRootImpl.java", callback.fileName)
        assertEquals("run", callback.methodName)
        assertEquals(-1, callback.lineNumber)

        val handler = trace.last()
        assertEquals(ActivityThread.H::class.java.name, handler.className)
        assertEquals("Handler.java", handler.fileName)
        assertEquals("handleMessage", handler.methodName)
        assertEquals(-1, handler.lineNumber)
